...
# 定义参数网格
param_grid = {
    'n_estimators': range(100, 200, 50),  # 从100到300，步长为50
    'learning_rate': [0.05, 0.1,  0.15],  # 从0.01到0.2
    'max_depth': range(3, 10),  # 从3到5
    # 'min_samples_split': range(2, 6),  # 从2到5
    # 'min_samples_leaf': range(1, 5),  # 从1到4
    'max_features': [None, 'sqrt', 'log2'],  # 不同的特征选择方法
...
for name, model in models.items():
    ...
    # 网格搜索
    grid_search = GridSearchCV(model, param_grid, cv=kfold, scoring='neg_mean_squared_error',verbose = 2,n_jobs=-1)
    grid_search.fit(X, Y)
