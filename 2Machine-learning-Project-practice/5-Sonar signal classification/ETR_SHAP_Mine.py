...
shap.summary_plot(shap_values[1], model.named_steps['scaler'].transform(X),
                 feature_names=feature_names, plot_type="dot",
                 show=False)
plt.title("SHAP Feature Importance (Class 'Mine')")
plt.tight_layout()
plt.show()
...
shap.summary_plot(shap_values[1], model.named_steps['scaler'].transform(X),
                 ...
                 show=False)
plt.title("Mean |SHAP Value| (Class 'Mine')")
plt.tight_layout()
plt.show()

# 3. 单个样本的SHAP决策图（展示第一个样本）
sample_idx = 0
...
plt.title(f"SHAP Decision Plot for Sample {sample_idx} (Class 'Mine')")
plt.tight_layout()
plt.show()

# 4. 特征重要性对比（两个类别的SHAP值）
shap.dependence_plot(0, shap_values[1], model.named_steps['scaler'].transform(X),
                    feature_names=feature_names, interaction_index=None)
plt.title("SHAP Dependence Plot for Top Feature")
plt.show()